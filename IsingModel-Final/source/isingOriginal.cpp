...
//do_step_wolf yerine do_step kullanýldý
...
//do_step_wolf içindeki recursive fonk(kullanýlmadý)
...
{   
    ...
    //end time
    cout<<"total  "<<endTime-startTime<<endl;
...
void do_step(bool spins[SIZE],  int neighs[SIZE][4], double tstar, int N, double h[5], double& energy, mt19937& gen, uniform_real_distribution<double>& ran_u, uniform_int_distribution<int>& ran_pos, double m[DATA])
{
	double doStepStart=omp_get_wtime();
	int i,j; //Counters
	...
	double doStepEnd=omp_get_wtime();
	cout<<"do_step took "<<doStepEnd-doStepStart<<endl;
	dsTime+=doStepEnd-doStepStart;
	return;
}
